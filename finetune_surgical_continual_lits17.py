...
from torch.utils.data import DataLoader, Dataset, Subset # --- MODIFIED ---: Imported Dataset and Subset
...
# --- MODIFIED ---: Renamed arguments for clarity and added new ones for the second dataset (LiTS)
parser.add_argument('--root_path_kits', type=str,
                    default='../data/kits23/train_npz', help='root dir for KITS23 data')
parser.add_argument('--list_dir_kits', type=str,
                    default='./lists/kits23', help='list dir for KITS23')
parser.add_argument('--root_path_lits', type=str, required=True,
                    help='root dir for LiTS17 data')
parser.add_argument('--list_dir_lits', type=str, required=True,
                    help='list dir for LiTS17')
parser.add_argument('--dataset', type=str,
                    default='kits23_lits17', help='experiment_name')
parser.add_argument('--num_classes_old', type=int,
                    default=9, help='number of classes in the old model (e.g., Synapse)')
parser.add_argument('--num_classes_kits', type=int,
                    default=4, help='number of classes in the KITS23 dataset (including background)')
parser.add_argument('--num_classes_lits', type=int, required=True,
                    help='number of classes in the LiTS17 dataset (including background)')
# Note: --num_classes_new is now conceptually split into --num_classes_kits and --num_classes_lits
...
# The original --num_classes_new is now calculated based on the new datasets
args.num_classes_new = 1 + (args.num_classes_kits - 1) + (args.num_classes_lits - 1)
...
def get_lr_weights(model, loader, args, criterion, num_classes_total): # --- MODIFIED ---
    ...
    for batch_data in partial_loader:
        ...
        # --- MODIFIED ---: Label mapping is now done in the dataset, so we use the label directly
...
# +++ ADDED +++: A new Dataset class to combine KITS and LITS and handle label mapping.
class CombinedDataset(Dataset):
    """
    Combines KITS23 and LiTS17 datasets and handles the label mapping for continual learning.
    - Background (0) is shared.
    - KITS labels (1, 2, 3) are mapped to the first new class indices.
    - LITS labels (1, 2) are mapped to indices immediately after KITS classes.
    """
    def __init__(self, kits_dataset, lits_dataset, num_classes_old, num_classes_kits):
        self.kits_dataset = kits_dataset
        self.lits_dataset = lits_dataset
        self.num_kits = len(self.kits_dataset)
        self.num_lits = len(self.lits_dataset)
        self.total_len = self.num_kits + self.num_lits

        self.num_classes_old = num_classes_old
        self.num_classes_kits = num_classes_kits
        self.kits_offset = self.num_classes_old - 1
        self.lits_offset = self.kits_offset + (self.num_classes_kits - 1)

    def __len__(self):
        return self.total_len

    def __getitem__(self, index):
        if index < self.num_kits:
            # This sample is from KITS23
            sample = self.kits_dataset[index]
            label = sample['label']
            # Map non-background labels
            label[label > 0] += self.kits_offset
            sample['label'] = label
        else:
            # This sample is from LiTS17
            sample = self.lits_dataset[index - self.num_kits]
            label = sample['label']
            # Map non-background labels
            label[label > 0] += self.lits_offset
            sample['label'] = label
        return sample
...
def trainer_continual_learning(args, model, old_model, snapshot_path):
    ...
    # --- MODIFIED ---: Calculate total classes based on all datasets
    num_classes_total = args.num_classes_old + (args.num_classes_kits - 1) + (args.num_classes_lits - 1)
    batch_size = args.batch_size * args.n_gpu

    # --- MODIFIED ---: Load both KITS23 and LiTS17 datasets
    db_transform = transforms.Compose([RandomGenerator(output_size=[args.img_size, args.img_size])])

    db_kits = Synapse_dataset(base_dir=args.root_path_kits, list_dir=args.list_dir_kits, split="train",
                              transform=db_transform, is_kits=True)
    # Assuming Synapse_dataset can handle LiTS with a flag like `is_lits=True` or similar logic
    db_lits = Synapse_dataset(base_dir=args.root_path_lits, list_dir=args.list_dir_lits, split="train",
                              transform=db_transform, is_lits=True) # You might need to adapt your Dataset class

    # Combine datasets
    db_train_full = CombinedDataset(db_kits, db_lits, args.num_classes_old, args.num_classes_kits)
    ...
    print(f"Using {subset_size}/{total_samples} samples ({args.data_fraction*100:.1f}%) for continual learning from KITS23 and LiTS17.")
    logging.info(f"Using {subset_size}/{total_samples} samples ({args.data_fraction*100:.1f}%) for continual learning from KITS23 and LiTS17.")
    logging.info(f"Old classes: {args.num_classes_old}, KITS classes: {args.num_classes_kits}, LiTS classes: {args.num_classes_lits}, Total: {num_classes_total}")
    ...
    for epoch_num in iterator:
        ...
        if args.auto_tune != "none":
            ...
            weights = get_lr_weights(model, trainloader, args, ce_loss, num_classes_total) # --- MODIFIED ---
        ...
        for batch_idx, sampled_batch in enumerate(trainloader):
            image_batch, label_batch_mapped = sampled_batch['image'], sampled_batch['label'] # --- MODIFIED ---
            image_batch, label_batch_mapped = image_batch.cuda(), label_batch_mapped.cuda()
            ...
            # --- MODIFIED ---: Label mapping is now handled by the CombinedDataset class.
            # The `label_batch_mapped` variable already contains the correctly mapped labels.
...
if __name__ == "__main__":
    ...
    # --- MODIFIED ---: Create continual learning model with expanded output for all new classes
    cl_model = ContinualLearningModel(net, args.num_classes_old, args.num_classes_new).cuda()

    # --- MODIFIED ---: Print updated continual learning configuration
    total_output_classes = args.num_classes_old + args.num_classes_new - 1
    print(f"\n=== Continual Learning Configuration ===")
    print(f"Old model classes: {args.num_classes_old}")
    print(f"KITS23 classes: {args.num_classes_kits} -> New FG classes: {args.num_classes_kits - 1}")
    print(f"LiTS17 classes: {args.num_classes_lits} -> New FG classes: {args.num_classes_lits - 1}")
    print(f"Total classes in expanded model: {total_output_classes}")
