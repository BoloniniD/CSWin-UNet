# finetune_tpgm_continual_3task.py
import argparse
import logging
import os
import random
...
# MODIFICATION: Updated help text for dataset argument
parser.add_argument('--dataset', type=str, default='kits23', choices=['kits23', 'lits17'], help='experiment_name')
parser.add_argument('--list_dir', type=str, default='./lists/kits23', help='list dir')
parser.add_argument('--num_classes', type=int, default=4, help='output channel of network for the current task')
...
def check_gradients(model, epoch, iter_num):
    ...
    if iter_num % 20 == 0:
        # This can be noisy, so let's not print it every time
        pass
...
def trainer_finetune(args, model, snapshot_path):
    ...
    # MODIFICATION: Dynamically set dataset flags
    # This assumes your Synapse_dataset class can handle is_lits flag.
    # If not, you may need to modify the dataset class itself.
    ...
    logging.info(f"--- Training on dataset: {args.dataset.upper()} ---")
...
if __name__ == "__main__":
    ...
    # MODIFICATION: Set task-specific parameters based on the --dataset argument
