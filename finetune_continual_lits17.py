# finetune_continual_multitask.py
import argparse
import logging
import os
...
# --- Argument Parser Setup (No changes needed here) ---
parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default='../data/lits17/train_npz', help='root dir for data of the CURRENT task') # <-- MODIFIED comment
parser.add_argument('--dataset', type=str,
                    default='lits17', help='experiment_name (e.g., kits23, lits17)') # <-- MODIFIED comment
parser.add_argument('--list_dir', type=str,
                    default='./lists/lits17', help='list dir for the CURRENT task') # <-- MODIFIED comment
parser.add_argument('--output_dir', type=str, help='output dir')
parser.add_argument('--num_classes_new', type=int,
                    default=3, help='output channel of the new task network') # <-- MODIFIED default for LiTS
parser.add_argument('--num_classes_old', type=int,
                    default=13, help='output channel of the old task network (sum of all previous tasks)') # <-- MODIFIED default and comment
...
parser.add_argument('--pretrained_path', type=str, required=True,
                    help='path to the model checkpoint from the PREVIOUS task') # <-- MODIFIED comment
...
# This calculation remains correct. It will now be 13 + 3 = 16 for the third task.
...
def load_pretrained_weights_continual(model, pretrained_path, num_classes_old, num_classes_new):
    ...
    where num_classes_old is the total number of classes from all previous tasks.
    """ # <-- MODIFIED docstring for clarity
    print(f"Loading pretrained model from {pretrained_path} for continual learning.")
    print(f"Classes in previous model: {num_classes_old}, New task classes: {num_classes_new}")
    ...
    with torch.no_grad():
        for key in output_layer_keys:
            if key in model_dict:
                ...
                # This logic correctly copies weights for all previous tasks
...
def trainer_continual(args, model, snapshot_path):
    ...
    # --- MODIFICATION HERE ---
    # This makes the dataset loading more general.
    # It assumes your `Synapse_dataset` class can handle these flags.
    # If your class doesn't have `is_lits`, you will need to add it,
    # similar to how `is_kits` is implemented.
    is_kits = (args.dataset == 'kits23')
    is_lits = (args.dataset == 'lits17')

    db_train_full = Synapse_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
                                    transform=transforms.Compose(
                                        [RandomGenerator(output_size=[args.img_size, args.img_size])]),
                                    is_kits=is_kits,
                                    is_lits=is_lits) # <-- MODIFIED to be general
    # --- END MODIFICATION ---
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # This label shifting logic is general and works for any number of old classes
            ...
            if iter_num % 20 == 0:
                ...
                # This visualization logic also works perfectly for the third task
